    ...
    if inputs_embeds is None:
        ...
        if pixel_values is not None:
            ...
            if n_image_tokens != n_image_features:
                ...
                print(f"Number of image features: {n_image_features}")
                print(f"Pixel values shape: {pixel_values.shape if pixel_values is not None else 'None'}")
                print(f"Image grid thw: {image_grid_thw}")
                print(f"Visual model type: {type(self.visual)}")
                print(f"Visual model dtype: {self.visual.dtype}")
                
                # Print image token positions
                ...
                # Print detailed input sequence info
                print(f"Input sequence length: {input_ids.shape[-1]}")
                unique_tokens, counts = torch.unique(input_ids, return_counts=True)
                print(f"Unique tokens and their counts: {dict(zip(unique_tokens.tolist(), counts.tolist()))}")
                
                # Save debug info to file
                import os
                debug_dir = "/tmp/verl_debug"
                ...
                }, f"{debug_dir}/image_mismatch_debug.pt")
                print(f"Debug data saved to: {debug_dir}/image_mismatch_debug.pt")
                print("=== END DEBUG ===\n")
