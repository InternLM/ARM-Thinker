...
class FSDPEngine(BaseEngine):
    ...
    def init_model(self):
        ...
        self.module, self.optimizer, self.lr_scheduler = self._build_model_optimizer(
            self.config
        )
    ...
    def _build_model_optimizer(self, config):
        ...
        if self.config.model.get("custom_chat_template", None) is not None:
            custom_chat_template = self.config.model.custom_chat_template

            # Check if it's a file path (ends with .jinja, .jinja2, .template, or contains path separators)
            if custom_chat_template.endswith((".jinja", ".jinja2", ".template")) or (
                "/" in custom_chat_template or "\\" in custom_chat_template
            ):
                try:
                    with open(custom_chat_template, "r", encoding="utf-8") as f:
                        custom_chat_template = f.read()
                    logger.info(
                        f"Loaded custom chat template from file: {self.config.model.custom_chat_template}"
                    )
                except (FileNotFoundError, IOError) as e:
                    logger.error(
                        f"Failed to load custom chat template from file {custom_chat_template}: {e}"
                    )
                    raise
        ...
        if config.model.get("enable_activation_offload", False):
            ...
            enable_activation_offloading(
                module, config.strategy, enable_gradient_checkpointing
            )
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.autocast(device_type=device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                # pad it back
                preds = pad_input(
                    preds_rmpad, indices=indices, batch=batch, seqlen=seqlen
                ).squeeze(-1)
            else:
                preds = self.module(
                    input_ids=input_ids,
    ...
    def infer_batch(
        ...
        data: DataProto,
        post_fn: Callable[
            [DataProto, torch.Tensor], tuple[torch.Tensor, dict[str, torch.Tensor]]
        ],
        ...
        for key, t_list in preds_list.items():
            ...
            if use_dynamic_bsz:
                ...
                assert len(indices) == t_concat.size(
                    0
                ), f"{len(indices)} vs. {t_concat.size()}"
    ...
    def optimizer_step(self):
        ...
        if isinstance(self.module, FSDP):
            ...
            grad_norm = fsdp2_clip_grad_norm_(
                self.module.parameters(), max_norm=self.config.grad_clip
            )
        else:
