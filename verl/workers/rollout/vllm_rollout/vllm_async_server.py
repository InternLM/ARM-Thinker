...
class AsyncvLLMServer(AsyncServerBase):
    ...
    async def init_engine(self):
        ...
        # print(f"self.max_model_len: {self.max_model_len}")
    ...
    async def generate(
        ...
        max_tokens = max(
            1,
            min(
                self.config.rollout.response_length,  # no less than config.response_length
                self.max_model_len - len(prompt_ids) - 1,  # leave space for EOS token
            ),
        )
        # logger.info(
        #     f"max_tokens = self.max_model_len - len(prompt_ids) = {self.max_model_len} - {len(prompt_ids)} = {max_tokens}"
        # )
        print(
            f"max_tokens = max(1, min(self.config.rollout.response_length, self.max_model_len - len(prompt_ids) - 1)) = {max_tokens}"
        )
        sampling_params["logprobs"] = (
            0 if sampling_params.pop("logprobs", False) else None
        )
        sampling_params = SamplingParams(max_tokens=max_tokens, **sampling_params)
        print(f"sampling_params in AsyncvLLMServer.generate(): {sampling_params}")
        ...
        # [TAG-Chris]
        # print token_ids
        # In Qwen2.5-VL, token </tool_call> is `151658`
        tool_call_end_token_id = 151658
        if tool_call_end_token_id in token_ids:
            token_ids = token_ids[:token_ids.index(tool_call_end_token_id) + 1] # +1 for including the tool_call_end_token_id
            print(f"original length: {len(token_ids)}, after truncating to first </tool_call>: {len(token_ids)}")
