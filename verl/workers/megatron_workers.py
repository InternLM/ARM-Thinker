...
class ActorRolloutRefWorker(MegatronWorker, DistProfilerExtension):
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        model_config: HFModelConfig = omega_conf_to_dataclass(
            omega_model_config, dataclass_type=HFModelConfig
        )
    ...
    def init_model(self):
        ...
        if self._is_actor:
            override_transformer_config = OmegaConf.to_container(
                OmegaConf.create(
                    self.config.actor.megatron.get("override_transformer_config", {})
                )
            )
            override_ddp_config = OmegaConf.to_container(
                OmegaConf.create(
                    self.config.actor.megatron.get("override_ddp_config", {})
                )
            )
        elif self._is_ref:
            override_transformer_config = OmegaConf.to_container(
                OmegaConf.create(
                    self.config.ref.megatron.get("override_transformer_config", {})
                )
            )
        else:
            override_transformer_config = {}
...
class AsyncActorRolloutRefWorker(ActorRolloutRefWorker):
    def _build_rollout(self, trust_remote_code=False):
        rollout_worker, rollout_sharding_manager = super()._build_rollout(
            trust_remote_code
        )
...
class CriticWorker(MegatronWorker, DistProfilerExtension):
    ...
    def init_model(self):
        ...
        override_transformer_config = OmegaConf.to_container(
            OmegaConf.create(
                self.config.megatron.get("override_transformer_config", {})
            )
        )
        override_ddp_config = OmegaConf.to_container(
