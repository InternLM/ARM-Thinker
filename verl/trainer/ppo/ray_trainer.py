...
class RayPPOTrainer:
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                timing_logger.info(
                    f"\n[Step {self.global_steps}] ========== START at {time.strftime('%H:%M:%S')} =========="
                )

                with marked_timer("step", timing_raw):
                    # generate a batch
                    timing_logger.info(
                        f"[Step {self.global_steps}] Phase 1/8: Generation START"
                    )
                    ...
                    timing_logger.info(
                        f"[Step {self.global_steps}] Phase 1/8: Generation DONE ({timing_raw.get('gen', 0):.1f}s)"
                    )
                    ...
                    timing_logger.info(
                        f"[Step {self.global_steps}] Phase 2/8: Reward START"
                    )
                    ...
                    timing_logger.info(
                        f"[Step {self.global_steps}] Phase 2/8: Reward DONE ({timing_raw.get('reward', 0):.1f}s)"
                    )

                    # recompute old_log_probs
                    timing_logger.info(
                        f"[Step {self.global_steps}] Phase 3/8: Old LogProb START"
                    )
                    ...
                    timing_logger.info(
                        f"[Step {self.global_steps}] Phase 3/8: Old LogProb DONE ({timing_raw.get('old_log_prob', 0):.1f}s)"
                    )
                    if self.use_reference_policy:
                        # compute reference log_prob
                        timing_logger.info(
                            f"[Step {self.global_steps}] Phase 4/8: Ref Policy START"
                        )
                        ...
                        timing_logger.info(
                            f"[Step {self.global_steps}] Phase 4/8: Ref Policy DONE ({timing_raw.get('ref', 0):.1f}s)"
                        )

                    # compute values
                    if self.use_critic:
                        timing_logger.info(
                            f"[Step {self.global_steps}] Phase 5/8: Values START"
                        )
                        with marked_timer("values", timing_raw, color="cyan"):
                            values = self.critic_wg.compute_values(batch)
                            batch = batch.union(values)
                        timing_logger.info(
                            f"[Step {self.global_steps}] Phase 5/8: Values DONE ({timing_raw.get('values', 0):.1f}s)"
                        )
                    ...
                    if self.use_critic:
                        timing_logger.info(
                            f"[Step {self.global_steps}] Phase 7/8: Critic Update START"
                        )
                        ...
                        timing_logger.info(
                            f"[Step {self.global_steps}] Phase 7/8: Critic Update DONE ({timing_raw.get('update_critic', 0):.1f}s)"
                        )
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        # update actor
                        timing_logger.info(
                            f"[Step {self.global_steps}] Phase 8/8: Actor Update START"
                        )
                        ...
                        timing_logger.info(
                            f"[Step {self.global_steps}] Phase 8/8: Actor Update DONE ({timing_raw.get('update_actor', 0):.1f}s)"
                        )

                    timing_logger.info(
                        f"[Step {self.global_steps}] ========== COMPLETE (total: {timing_raw.get('step', 0):.1f}s) ==========\n"
                    )

                    # Log rollout generations if enabled
                    rollout_data_dir = self.config.trainer.get("rollout_data_dir", None)
                    print(f"[DEBUG] rollout_data_dir: {rollout_data_dir}")
                    if rollout_data_dir:
                        print(f"[DEBUG] dump rollout generations")
                        with marked_timer(
                            ...
                            # inputs = self.tokenizer.batch_decode(
                            #     batch.batch["prompts"], skip_special_tokens=False
                            # )
                            # outputs = self.tokenizer.batch_decode(
                ...
                print(
                    f"[DEBUG] gen_batch_output.meta_info keys: {gen_batch_output.meta_info.keys()}"
                )

                if "metrics" in gen_batch_output.meta_info:
                    print(f"[DEBUG] Found metrics in gen_batch_output.meta_info")
                    rollout_metrics = []
                    for chunk_metrics in gen_batch_output.meta_info["metrics"]:
                        rollout_metrics.extend(chunk_metrics)
                    print(f"[DEBUG] Collected {len(rollout_metrics)} rollout metrics")
                    tool_metrics = self._compute_tool_metrics(rollout_metrics)
                else:
                    print("[DEBUG] No metrics found in gen_batch_output.meta_info")
                    tool_metrics = {
                        "tool_call_cnt/mean": 0.0,
                        "tool_call_cnt/success_rate": 0.0,
                    }

                print(f"[DEBUG] computed tool_metrics: {tool_metrics}")
    ...
    def _compute_tool_metrics(self, rollout_metrics: list[dict]) -> dict[str, float]:
        """Compute tool-related metrics from rollout data.

        Args:
            rollout_metrics: List of metrics from each rollout in the batch

        Returns:
            Dictionary containing average tool call count and success rate
        """
        total_tool_calls = 0
        ...
        print(f"[DEBUG] rollout_metrics type: {type(rollout_metrics)}")
        print(
            f"[DEBUG] rollout_metrics length: {len(rollout_metrics) if isinstance(rollout_metrics, list) else 'not a list'}"
        )

        for i, rollout_metric in enumerate(rollout_metrics):
            print(f"[DEBUG] rollout_metric[{i}] type: {type(rollout_metric)}")
            print(f"[DEBUG] rollout_metric[{i}] content: {rollout_metric}")

            if isinstance(rollout_metric, dict):
                tool_calls = rollout_metric.get("tool_calls_count", 0)
                successful_calls = rollout_metric.get("successful_tool_calls", 0)
                failed_calls = rollout_metric.get("failed_tool_calls", 0)

                print(
                    f"[DEBUG] rollout[{i}] - tool_calls: {tool_calls}, successful: {successful_calls}, failed: {failed_calls}"
                )
        ...
        if valid_rollouts == 0:
            return {"tool_call_cnt/mean": 0.0, "tool_call_cnt/success_rate": 0.0}

        avg_tool_calls = total_tool_calls / valid_rollouts

        # Calculate success rate: successful calls / total calls
        # If no tool calls, success rate is 0
        if total_tool_calls == 0:
            success_rate = 0.0
        else:
            success_rate = total_successful_calls / total_tool_calls

        print(
            f"[DEBUG] Final computation - total_tool_calls: {total_tool_calls}, valid_rollouts: {valid_rollouts}"
        )
        print(
            f"[DEBUG] Final computation - avg_tool_calls: {avg_tool_calls}, success_rate: {success_rate}"
        )

        return {
            "tool_call_cnt/mean": avg_tool_calls,
            "tool_call_cnt/success_rate": success_rate,
        }
